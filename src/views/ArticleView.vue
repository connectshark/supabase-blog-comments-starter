<template>
  <main class=" container max-w-2xl mx-auto">
    ...
    <article v-else>
      <h1 class=" py-10 text-center text-2xl text-white bg-primary   shadow-[0_0_0_100vmax_#7a6ae1] path">{{ results.title
      }}</h1>
    ...
    <template v-if="results.user_id === store.id">
      ...
      <div>
        <p class="py-3">
          <input v-model="is_published" :disabled="updateLoading" @change="updateArticleHandler" type="checkbox" id="Is_published"><label class=" inline-block ml-4" for="Is_published">是否發佈</label>
        </p>
      </div>
    </template>
    ...
    <ul>
      ...
      <li v-for="comment in comments" :key="comment.user_id" class=" bg-gray-200 p-5 mb-5">
        ...
        <p class=" text-right" v-if="store.id && store.id === results.user_id || store.id && store.id === comment.user_id">
